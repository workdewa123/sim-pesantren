    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-xs">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
            <div>
                ...
                <p class="text-slate-400 mt-0.5">Kelola seluruh konten tulisan kegiatan santri dan siaran resmi pondok
                    pesantren.</p>
            </div>
            <button type="button" onclick="openModal('modalCreate')"
                    ...
                    {{-- @forelse($kegiatan as $index => $row) --}}
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="p-3 text-slate-500">
                                {{-- {{ \Carbon\Carbon::parse($row->tanggal_kegiatan)->translatedFormat('d M Y') }}</td> --}}
                            <td class="p-3 font-medium text-slate-500"><i
                                    {{-- class="fa-regular fa-user text-[10px] mr-1 text-slate-400"></i>{{ $row->penulis }}</td> --}}
                            <td class="p-3 flex justify-center gap-1.5 mt-1.5">
                                ...
                                {{-- <button type="button" onclick="editKegiatan({{ $row->id }})" --}}
                                    class="p-2 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg cursor-pointer"
                                    title="Koreksi"><i class="fa-solid fa-pen-to-square"></i></button>
                                {{-- <form action="{{ route('media.kegiatan.destroy', $row->id) }}" method="POST"
